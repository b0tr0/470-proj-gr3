...
export default function Fuel() {
  ...
  const handleDeleteFuel = async (id) => {
    if (!window.confirm('Are you sure you want to delete this fuel report?')) return;
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://localhost:5000/api/fuel/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      alert('Fuel status report deleted successfully!');
      fetchFuelData();
    } catch (err) {
      console.error('Delete fuel report error:', err);
      alert(err.response?.data?.message || 'Failed to delete fuel report.');
    }
  };
      ...
      <div style={{ height: '350px', width: '100%', borderRadius: '12px', overflow: 'hidden', marginBottom: '24px' }}>
        <MapContainer center={[23.8103, 90.4125]} zoom={11} style={{ height: '100%', width: '100%' }}>
          ...
          {fuelStations.map((station) => {
            const authorId = (station.reportedBy?._id || station.reportedBy || '').toString();
            const canDeleteStation = Boolean(currentUserId && authorId === currentUserId) || isPrivileged;
            const stationId = station._id || station.id;

            return (
              <Marker 
                key={stationId} 
                position={[station.location?.lat || 23.8103, station.location?.lng || 90.4125]} 
                icon={customIcon}
              >
                <Popup>
                  <div style={{ color: '#000' }}>
                    ...
                    ⏳ Queue: {station.queueLength}<br />
                    ⏰ Expires at: {formatExpiry(station.expiresAt)}
                    {canDeleteStation && (
                      <div style={{ marginTop: '8px' }}>
                        <button
                          type="button"
                          onClick={() => handleDeleteFuel(stationId)}
                          style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {fuelStations.length === 0 ? <p style={{ color: '#94a3b8' }}>No fuel updates found.</p> : (
            fuelStations.map((item) => {
              ...
              const authorId = (item.reportedBy?._id || item.reportedBy || '').toString();
              const canDeleteCard = Boolean(currentUserId && authorId === currentUserId) || isPrivileged;

              return (
                <div key={itemId} style={{ backgroundColor: '#0d3326', padding: '14px', borderRadius: '8px', borderLeft: '4px solid #10b981' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      {canDeleteCard && (
                        <button
                          type="button"
                          onClick={() => handleDeleteFuel(itemId)}
                          style={{
                            backgroundColor: '#ef4444',
                            color: '#ffffff',
                            border: 'none',
